...
export class UserInitComponent implements OnInit {
  ...
  calculateOrders() {
    this.ordNotApproved = this.orders.filter(ord => ord.status === 'waiting for approve').length;
    this.ordApproved = this.orders.filter(ord => ord.status === 'Approved').length;
    this.ordCompleted = this.orders.filter(ord => ord.status === 'Arrive - Complete').length;
    this.ordPaid = this.orders.filter(ord => ord.status === 'Paid' || ord.status === 'Shipped' || ord.status === 'On its way').length;
  }
